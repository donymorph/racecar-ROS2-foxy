...
{
    int wRet;
    unsigned char wificmd0[] = { 0xa5, 0x3A, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
    unsigned char wificmd1[] = { 0xa5, 0x2C, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
    unsigned char wificmd2[] = { 0xa5, 0x20, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };

    ALOGI("--------start scan----------\n");

    wRet = write(m_dFd, wificmd0, 7);
    if (wRet < 0) {
        perror("Failed to send wificmd0");
        return wRet;
    }
    usleep(5000000);
    ...
    if (wRet < 0) {
        perror("Failed to send wificmd1");
        return wRet;
    }
    usleep(3000);
    ...
    if (wRet < 0) {
        perror("Failed to send wificmd2");
        return wRet;
    }
...
int io_driver::OpenSerial(const char* port, unsigned int baudrate)
{
    ...
    m_stNew = m_stOld;
    cfmakeraw(&m_stNew);  // Set terminal to raw mode
    ...
    m_stNew.c_cc[VTIME] = 0;  // Set timeout
    m_stNew.c_cc[VMIN] = 1;   // Set minimum number of characters

    tcflush(m_dFd, TCIFLUSH);  // Flush terminal
...
int io_driver::StartScan(void)
{
    ...
    unsigned char wificmd1[] = { 0xa5, 0x2C, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
    unsigned char wificmd2[] = { 0xa5, 0x20, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
    unsigned char wificmd3[] = { 0xa5, 0x50, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
...
{
    unsigned char buf[] = { 0xa5, 0x40, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
...
{
    unsigned char stop_scan[] = { 0xa5, 0x21, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
    unsigned char stop_motor[] = { 0xa5, 0x25, 0xe1, 0xaa, 0xbb, 0xcc, 0xdd };
